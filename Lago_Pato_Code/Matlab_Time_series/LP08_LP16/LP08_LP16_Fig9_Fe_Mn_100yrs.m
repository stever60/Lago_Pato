...
load LP08_Fig9_Fe_Mn_SH20M1H.txt
series1 = LP08_Fig9_Fe_Mn_SH20M1H(all(~isnan(LP08_Fig9_Fe_Mn_SH20M1H),2),:);
%Series 2
load LP16_Fig9_Fe_Mn_SH20M1H.txt
series2 = LP16_Fig9_Fe_Mn_SH20M1H(all(~isnan(LP16_Fig9_Fe_Mn_SH20M1H),2),:);
...
factor1 = meanyr1*(100/meanyr1)
factor2 = meanyr2*(100/meanyr2)
meanyr1x = factor1
meanyr2x = factor2
...
x1reshape = reshape(t1,[100,1]);
x2reshape = reshape(t2,[131,1]);

y1reshape = reshape(s1inter1,[100,1]);
y2reshape = reshape(s2inter1,[131,1]);
y1normreshape = reshape(s1norminter,[100,1]);
y2normreshape = reshape(s2norminter,[131,1]);
...
%Series 1
x2s = reshape(x2,[995,1]);
y2s = reshape(y2,[995,1]);
y2sl = reshape(y2l,[995,1]);
y2sln = reshape(y2ln,[995,1]);
